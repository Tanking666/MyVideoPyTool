...
class Video(File):
    def __init__(self, path):
        super().__init__(path)
        if self.path != '':
            if self.exten in ("mkv", "mp4", "avi", "rmvb"):
                self.type = "video"
                self.id = ""
                self.length = 0
    ...
    def save_or_update(self):
        if self.path != '':
            ...
            if len(data) > 0:
                ...
                self.get_video_info()
                # self.get_md55()
                sql = 'INSERT INTO video (id, name, p_name, size, path, file_type, `len`,h,w,fps,file_name) VALUES ( "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", %s, "%s");' % (
                          self.id, self.name, self.p_name, self.size, self.path, self.exten,
                          self.length, self.h, self.w, self.fps, self.path)
                try:
                    mysql.execute(sql)
                    print("[INFO] INSERT:  " + self.path)
                except BaseException:
                    print('[ERROR]sql=>' + sql)
...
if __name__ == '__main__':
    v = Video(
        r"X:/Video/2合集/[2006-2010]/[2006][Fate_Stay_Night+UBW+Fate_Zero 合集][BDRIP][720P][X264-10bit_AAC]/[异域-11番小队][Fate_Unlimited_Blade_Works][00-25+MOVIE+GE+SP][BDRIP][720P][X264-10bit_AAC]/[Fate Unlimited Blade Works][15][BDRIP][X264][10bit][AAC][00][BDRIP][D919].mp4")
    v.save_or_update()
